...
export class MemStorage implements IStorage {
  ...
  async createOrUpdateSettings(insertSettings: InsertSettings): Promise<Settings> {
    const settings: Settings = {
      ...
      aiModel: insertSettings.aiModel ?? "gpt-4o-mini",
      customModelId: insertSettings.customModelId ?? null,
      aiSystemPrompt: insertSettings.aiSystemPrompt ?? "You are a helpful WhatsApp assistant for our business. Provide concise, friendly, and professional responses to customer inquiries. Keep responses brief and relevant to their questions.",
  ...
  async createCampaign(insertCampaign: InsertCampaign): Promise<Campaign> {
    ...
    const campaign: Campaign = {
      ...
      mediaId: insertCampaign.mediaId ?? null,
